...
@Component
public class ZongRequestProcessing {
    ...
    private static Long tempIdforUpdate = 0L;
    ...
    public ZongRequestProcessing() {}
    public ZongRequestProcessing(BlacklistService blacklist, TodayChargedMsisdnRepository repository, ChargingService chargingService, TodayChargedMsisdnService todayChargedMsisdnService, ModelMapper modelMapper) {
    ...
    public void processZongRequest(JmsMessage incomingPacket) throws IOException {
        ...
        log.info("User does not exist in today_tbl_billing | Starting Process for Charging");
        ...
        // firstly added in today_tbl_billing
        mydao2= new TodayChargedEntity(incomingPacket.getSmsId(),
                new_subscriberNumber2, String.valueOf(incomingPacket.getSmsId()),
                8888, incomingPacket.getShortcode(),
                scKeyword2, 8888, Constants.PARTNER_ID, 8888, 0, 8888,
                incomingPacket.getConnectivityPointId(), incomingPacket.getSmsText(), charging_mechanism, 8888);


        tempIdforUpdate=  todayChargedMsisdnService.insertRecord(mydao2);
        ...
        if (bl != null) {
            ...
            if (status == 0 && blid == 0) {
                ...
                if (code.equalsIgnoreCase("0000") || code.equalsIgnoreCase("0") || code.equalsIgnoreCase("000")) {
                    ...
                    chargingService.insertRecord(mydao);
                    todayChargedDTO = modelMapper.map(mydao, TodayChargedDTO.class);
//                    log.info(String.valueOf(todayChargedDTO));
//                    log.info(tempIdforUpdate.toString());
                    todayChargedMsisdnService.updateRecord(todayChargedDTO, tempIdforUpdate);
